...
test('publishes version 13.2.0 with a complete offline shell', async () => {
  const worker = loadServiceWorker();

  assert.equal(worker.context.self.IPMAX_VERSION, '13.2.0');
  assert.equal(worker.context.self.IPMAX_CACHE_NAME, 'ipmax-v13.2.0');
...
test('deletes only stale Interview Prep Max caches on activation', async () => {
  const worker = loadServiceWorker(['ipmax-v13.1.0', 'ipmax-v13.2.0', 'another-app-v4']);
...
test('serves every release bootstrap file without HTTP caching', async () => {
  ...
  try {
    ...
    assert.match((await request(server, '/version.js')).body, /IPMAX_VERSION = '13\.2\.0'/);
  } finally {
...
test('passes the release integrity verifier', () => {
  ...
  assert.match(result.stdout, /Release 13\.2\.0 integrity check passed/);
});
