...
test('keeps curriculum and video flashcards as separate visible decks', async ({ page }) => {
  ...
  await expect(studyDeck.locator('strong')).toHaveText('2329');
...
test('defers exam cards and renders the full list in batches', async ({ page }) => {
  ...
  await expect(page.locator('#questions-load-more')).toContainText('12/921');
...
test('flat flashcard categories preserve progress, filter independently and keep keyboard focus', async ({ page }) => {
  ...
  const linux = categories.getByRole('button', { name: 'Linux и Bash 367', exact: true });
  await linux.click();
  ...
  await expect(linux.locator('strong')).toHaveText('367');
  await search.fill('');
